...
from network.xception_7layers import xception
    ...
    for epoch in range(current_epoch, num_epochs):
        ...
        for phase in ['train', 'test']:
            ...
            for i, (inputs, labels) in enumerate(dataloaders[phase]):
                ...
                if (i + 1) % 300 == 0 and phase == 'train':
                    print_mem()
                    inter_loss = running_loss_train / 300.0
...
if __name__ == '__main__':
    ...
    current_epoch = 0
    batch_size = 32
    train_csv = "../../csv/dfgc_train.csv"    # The train split file
    val_csv = "../../csv/dfgc_val.csv"        # The validation split file

    #  Output path
    model_dir = '../training_dwt2_3'
    ...
    log.write('model : xception   batch_size : 32 frames : 10 \n')
    log.write('pretrain : True   input_size : 64*64*7\n')
    ...
    train_set, valid_set = Dataset_Csv(train_list, train_label, transform=xception_transforms_train), \
                           Dataset_Csv(test_list, test_label, transform=xception_transforms_train, train=False)
    ...
    optimizer_ft = optim.Adam(model.parameters(), lr=0.00015, weight_decay=0.001)
    exp_lr_scheduler = lr_scheduler.StepLR(optimizer_ft, step_size=2, gamma=0.5)

    writer = SummaryWriter('runs/xception_experiment_7_layers')
